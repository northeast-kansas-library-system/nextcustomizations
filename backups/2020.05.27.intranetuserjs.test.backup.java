...
*/
 ...
 //BEGIN re-logo the staff client
  //All pages-url for logo https://nekls.org/wp-content/uploads/2018/03/nekls_next_logo.png
   $("#header_search").before('<div id="newlogo" style="float: left; padding: 20px; height: 88;"  ><a href="/cgi-bin/koha/mainpage.pl"><img src="https://nekls.org/wp-content/uploads/2018/03/nekls_next_logo.png" style="max-height: 50px"><p style="font-size: 75%;">A service of NEKLS</p></a></div>');
...
//KOHA > * (all pages with breadcrumbs)
 //BEGIN add message to all screens with breadcrumbs - requires css to hide ids and classes (good for upgrade notes, etc.)
  $("#breadcrumbs").prepend("<h4 id='upgrade_note1' class='upgrade1' style='display: none'>NEXT Search Catalog will be upgraded on the evening of DAY, MONTH DATE, YEAR.<br />While the catalog is being upgraded patrons will not be able to access the OPAC.<br />Look at the top of the NEWS column on the left hand side of the NEXT homepage for links to our training website, printable training materials, and training videos.</font></h4><h4 id='upgrade_note2' class='upgrade1' style='display: none;'><font color='#ff0000'>NEXT Search Catalog was upgraded on the evening of DAY, MONTH DATE, YEAR.<br /><br />Remember that you must clear your Firefox cache (type ctrl-shift-delete while you have Firefox open)<br />the first time you log in after the upgrade in order to make sure there are no old pages interfering with the upgraded software.</font></h4>");
  $('.upgrade1, .upgrade2').hover(function() {
   $('.upgrade1, .upgrade2').css('background-color','#FFFF00');
   }, function(){
...
//BEGIN reorganize and enchance add/edit item page
 //add classes to additem.pl fields
  ...
  $('#cat_additem span:contains("Materials specified (bound volume or other part)")').parent().parent().parent().addClass('iaspecified emb');
  ...
  $('#cat_additem span:contains("ollection")').parent().parent().parent().addClass('iaccode emb');
  ...
  $('#cat_additem span:contains("arcode")').parent().parent().parent().addClass('iabcode emb');
  ...
  $('#cat_additem span:contains("Koha item type")').parent().parent().parent().addClass('iaitype emb');
...
$('#circ_circulation #message_type option[value="B"]').attr('selected','selected');
$('#circ_circulation #borrower_message').val('All requests for pickup at Tonganoxie Public Library have been temporarily suspended due to the ongoing closure at Tonganoxie Public Library.  The requests will be automatically resumed when the library reopens.');
...
$('#cat_additem input[id^="tag_952_subfield_p').on('keyup', function(e) {
    $(this).val($(this).val().toUpperCase());
});

$('.problem:contains("New value:")').wrapInner('<span></span>')
