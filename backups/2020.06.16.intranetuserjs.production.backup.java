...
//Catalog > Detalis for -TITLE-
 ...
 //Enhance "Recently returned" shelving location
...
$('#circ_circulation #specify-due-date #cleardate').after('<br /><h3>If you need to modify a due date,<br />use these controls to set a new due date <span style="font-style: italic; text-decoration: underline;">before</span> you scan an item barcode.</h3>');
 ...
 //Partially open
  $('.BALDWIN #breadcrumbs, .BASEHOR #breadcrumbs, .BERN #breadcrumbs, .BONNERSPGS #breadcrumbs, .BURLINGAME #breadcrumbs, .CARBONDALE #breadcrumbs, .EUDORA #breadcrumbs, .HOLTON #breadcrumbs, .HORTON #breadcrumbs, .LEAVENWRTH #breadcrumbs, .LINWOOD #breadcrumbs, .MCLOUTH #breadcrumbs, .MERIDEN #breadcrumbs, .NORTONVLLE #breadcrumbs, .OSAWATOMIE #breadcrumbs, .OTTAWA #breadcrumbs, .OVERBROOK #breadcrumbs, .PAOLA #breadcrumbs, .POMONA #breadcrumbs, .ROSSVILLE #breadcrumbs, .TONGANOXIE #breadcrumbs, .VALLEYFALL #breadcrumbs, .WETMORE #breadcrumbs, .WILLIAMSBG #breadcrumbs, .WINCHESTER #breadcrumbs').prepend('<h1 class="closed partlyclosed" style="text-align: center;">Your library is currently listed as <span style="font-style: italic; text-decoration: underline;">partially open</span> on the OPAC.  Please contact <a href="mailto:nexthelp@nekls.org">nexthelp@nekls.org</a> when you open fully to the public.</h1>');
  $('.partlyclosed').parent().hover(function() {
   $('body, .partlyclosed').css('background-color','#b3ffb3');
   }, function(){
   $('.partlyclosed').css('background-color','#E6F0F2');
   $('body').css('background-color','#ffffff')
  });

 //Closed till June 22
  $('.PERRY #breadcrumbs').prepend('<h1 class="closed junetwentytwo" style="text-align: center;">Your library is currently listed as <span style="font-style: italic; text-decoration: underline;">Closed until June 22</span> on the OPAC.  Please contact <a href="mailto:nexthelp@nekls.org">nexthelp@nekls.org</a> when you are ready to reopen.</h1>');
  $('.junetwentytwo').parent().hover(function() {
   $('body, .junetwentytwo').css('background-color','#ffff99');
   }, function(){
   $('.junetwentytwo').css('background-color','#E6F0F2');
   $('body').css('background-color','#ffffff');
  });

 //Closed till August
  $('.HIAWATHA #breadcrumbs, .PHAXTELL #breadcrumbs, .PHSES #breadcrumbs, .PHSMS #breadcrumbs, .PHSHS #breadcrumbs, .PHWAC #breadcrumbs').prepend('<h1 class="closed augustfirst" style="text-align: center;">Your library is currently listed as <span style="font-style: italic; text-decoration: underline;">Closed until August</span> on the OPAC.  Please contact <a href="mailto:nexthelp@nekls.org">nexthelp@nekls.org</a> when you are ready to reopen.</h1>');
  $('.augustfirst').parent().hover(function() {
   $('body, .augustfirst').css('background-color','#ffff99');
   }, function(){
   $('.augustfirst').css('background-color','#E6F0F2');
   $('body').css('background-color','#ffffff');
  });


 //Closed till further notice
  $('.HIGH_CC #breadcrumbs').prepend('<h1 class="closed closedtba" style="text-align: center;">Your library is currently listed as <span style="font-style: italic; text-decoration: underline;">Closed until further notice</span> on the OPAC.  Please contact <a href="mailto:nexthelp@nekls.org">nexthelp@nekls.org</a> when you are ready to reopen.</h1>');
  $('.closedtba').parent().hover(function() {
   $('body, .closedtba').css('background-color','#ffb3b3');
   }, function(){
   $('.closedtba').css('background-color','#E6F0F2');
   $('body').css('background-color','#ffffff');
  });
...
/*
 $('#holds-table').on( 'init.dt', function () {
  $('.hold_location_select:contains("Hiawatha, Morrill Public Library (current)")').parent().parent().attr('class','hiarow');
  $('.hiarow [id^="resume"]').hide();
 });
*/
...
/*
 ...
 //BEGIN add shortbranch codes
  var sbranchcode = $(".logged-in-branch-code").first().text().trim().substring(0,4);
 //Begin store logged in branchname as var
  var loglibname = $(".logged-in-branch-name:first").text().trim();
 //Begin store logged in branchname as var
  var sloglibname = $(".logged-in-branch-name:first").text().trim().substring(0,3);

 //BEGIN limit club creation to logged in library (needs refinement for districts)
  $('#club-template-branchcode option[value='+ branchescode +']').attr('selected','selected');
  $('#club-template-branchcode option[value!='+ branchescode +']').hide();
  $('#club-template-branchcode option[value^='+ sbranchcode +']').show();
